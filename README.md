...
1. Explain the difference between Relational Databases and SQL.
   Relational databases are made up of tables (rows and columns) of data, which is exactly what makes it a relational database. SQL is a standard language that makes it possible to manage databases with its management tools.

2. Why do tables need a Primary Key?
   A primary key is needed so that a table's rows can have a unique identifier for reference.

3. What is the name given to a table column that references the Primary Key on another table?
   A foreign key.

4. What do we need in order to have a _many to many_ relationship between two tables?
   A bridge table with 2 foreign keys connecting to the primary keys of the other two tables.
